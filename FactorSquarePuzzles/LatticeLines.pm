...
sub add_line {               # adds a line and updates the min and max
    my $self                    = shift;
    my $endpoints               = shift;         # string e.g. '0,1,1,0'; these are coefficients of basis vectors
...
sub add_clue_text {
    ...
    $self->{'clues'}->{$position} = $text_string;    #overwrites any existing text at same position.
}
...
sub add_answer_text {
  ...
  if(ref($position) eq 'ARRAY' ){ # stringify
    ...
    $self->{'answers'}->{$position} = $text_string;    #overwrites any existing text at same position.
}
...
sub clues_svg {
    ...
    foreach my $clue_position ( keys %position_clue ) {
        ...
        $text_svg_string .= $self->text_svg( $clue_text, \@position_array, $offset, $scale );
    }
...
sub answers_svg {
    ...
    foreach my $answer_position ( keys %position_answer ) {
        ...
        $text_svg_string .= $self->text_svg( $answer_text, \@position_array, $offset, $scale );
    }
    return $text_svg_string;
}
...
#   print STDERR "font size: $font_size , scale: $scale \n";
    ...
    $svg_text_string .=
      ' font-size="' . $font_size . '" style="text-anchor:' . $text_anchor . '" > ' . $text . "</text>\n";
...
sub line_svg {    # takes euclidean endpoints a and b
    ...
    $b = subtract_V( $b, [ $self->{min_x}, $self->{min_y} ] ); # so LL corner of bounding box is at origin
