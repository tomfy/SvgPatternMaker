...
my $what_to_show = $opt_w || 'clues'; # by default show the clues but not the answers. 'answers' 'both' are other possibilities.
my $show_clues   = 1;
my $show_answers = 0;
...
for my $row ( 0 .. $n_rows-1 ) {
    for my $col ( 0 .. $n_cols-1 ) {
        ...
        # svg body:
        $svg_string .= $puzzle_obj->svg_string( [$scale * $x_off, $scale * ($y_off+5)], $scale, { show_clues => $show_clues, show_answers => $show_answers, show_arrows => $show_arrows } );
...
#    my $scale          = shift || 100;
 ...
 my $clue_L5 = $entries[5] * $entries[6] * $entries[7];
  ...
  my $clue_I7 = $entries[0] * $entries[5] * $entries[10];
    ...
    my $answer_dy = -0.1;
    my $clue_dy = -0.1;
    $LLobj->add_clue_text( $clue_T1, [0.5,6.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_T2, [1.5,6.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_T3, [2.5,6.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_T4, [3.5,6.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_T5, [4.5,6.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_T6, [5.5,6.5 + $clue_dy] );

  $LLobj->add_clue_text( $clue_R1, [6.5,5.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_R2, [6.5,4.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_R3, [6.5,3.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_R4, [6.5,2.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_R5, [6.5,1.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_R6, [6.5,0.5 + $clue_dy] );

  $LLobj->add_clue_text( $clue_B1, [5.5,-0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_B2, [4.5,-0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_B3, [3.5,-0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_B4, [2.5,-0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_B5, [1.5,-0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_B6, [0.5,-0.5 + $clue_dy] );

    $LLobj->add_clue_text( $clue_L1, [-0.5,0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_L2, [-0.5,1.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_L3, [-0.5,2.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_L4, [-0.5,3.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_L5, [-0.5,4.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_L6, [-0.5,5.5 + $clue_dy] );

# interior clues
    $LLobj->add_clue_text( $clue_I1, [2.5,5.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I2, [3.5,4.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I3, [5.5,3.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I4, [4.5,2.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I5, [3.5,0.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I6, [2.5,1.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I7, [0.5,2.5 + $clue_dy] );
    $LLobj->add_clue_text( $clue_I8, [1.5,3.5 + $clue_dy] );
